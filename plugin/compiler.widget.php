...
function smarty_compiler_widget($arrParams,  $smarty){
    ...
    $strType = explode('.',$strName);
    $strType = end($strType);
    $strType = str_replace(array("_","=","+","\"","'"),"",$strType);
    ...
    if($strName){
        ...
        } elseif ($strType == 'js' || $strType == 'css') {
            $strCode .= 'FISResource::load('.$strName.', $_smarty_tpl->smarty);';
        } else {
            var_dump($strType);
            exit();
            $strCode .= 'trigger_error(\'error type of locale resource "\'.' . $strName . '.\'"\', E_USER_ERROR);';
        }
    } else {
